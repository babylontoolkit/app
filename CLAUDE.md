...
- **BINARY FILES ARE FIRST-CLASS:** every file path — template mount, git/folder import, WebContainer writes, snapshots, restore, GitHub sync, share builds, asset adds — carries binaries as bytes losslessly (base64/ArrayBuffer → `fs.writeFile` with `Uint8Array`). Binary CONTENT never enters LLM context or the editor's text map (store `isBinary` + size only). Upstream's text-oriented file layer MUST be extended, not worked around (SPEC §1.3 principle 10, §4.4).
