...
2026-07-31, at its predecessor Opus 4.8's exact price) **$2 / $10**; Fable 5 **$4 / $20** (the SuperMax rung's model until 2026-08-08 and the **Platinum** rung's default since 2026-08-10; still priced, and reachable by pointing `PREMIUM_MODEL` at it, which the owner's deploy does). Cache rates always DERIVE from the row — 0.1× read, **2×** write (the 1-hour tier,
...
| `ENABLE_EXTENDED_MODELS` | `true` | §4.6.1a master switch for the paid rung. `false` serves Standard alone. ⚠️ **The name has flipped TWICE** — `ENABLE_EXTENDED_MODELS` → `ENABLE_PREMIUM_MODEL` (2026-08-08, when the third rung was retired) → back to `ENABLE_EXTENDED_MODELS` (2026-08-10, when `platinum` restored it). **`ENABLE_PREMIUM_MODEL` is the REFUSED name today**; this row said the inverse until 2026-08-11. Take the polarity from `premium-model-flag.ts`, never from a document. The old nam — and the retired `SUPERMAX_MODEL` / `SUPERMAX_MINIMUM_CREDITS` — are **REFUSED if set**, because this flag defaults ON and a stale `false` would silently start serving the paid rung to everyone. An unrecognised, unpriceable or unaffordable rung resolves DOWN to Standard — never up |
