...
| `app/routes/api.projects.$projectId.snapshots.ts`, `…snapshots.$snapshotId.ts` | **DELETED (2026-07-16).** Both shipped narrowed — POST 405ing after the walls, the GET kept for remix seeds — which left an uncalled route that stores whole projects one call site from returning. The seed moved to its own read-only route (`…seed.ts`) and these are gone. |
...
| `app/routes/api.remix.ts` | Files now come from the caller (self-remix) or the published seed (shared remix) — the old snapshot pointer became `undefined` for every ordinary project when server snapshots went away, which silently cloned empty projects. |
...
| *(new)* `app/lib/.server/git/**`, `app/lib/.server/share/{remix-seed,seed-store}.ts`, `app/routes/api.projects.$projectId.seed.ts`, `app/lib/persistence/{local-snapshots,mount-source,save-queue,save-status,dependencies}.ts`, `app/components/persistence/**`, `app/config/saving.ts`, `app/routes/help.saving-projects.tsx`, `supabase/migrations/0006_*.sql`, `0007_drop_server_snapshots.sql` | All additive. No upstream file involved. |
| `app/lib/.server/projects/{store,types,ownership}.ts` | `SnapshotStore`, `FsSnapshotStore`, `SupabaseSnapshotStore`, `Snapshot`, `ManifestEntry`, `SnapshotPayload`, `buildManifest`, `snapshotKey`, `assertSnapshotBelongsTo` all **deleted**; `Project.currentSnapshotId` → `remixSeedAt`. The platform stores no project files (§4.5.4b), so none of it had a caller left once the remix seed moved to `share/seed-store.ts`. |
...
## Upstream files touched — 2026-07-18/19 (§4.16 media, model tier UX, §2.3 dead knobs, layout fixes)
...
| `app/components/chat/Chat.client.tsx` | Additive: `media-task` handler (client poller, latched — one delivery/toast per task), `creationTurnStore` derivation effect (§4.6.1a paid rungs are edit-only), `CREATION_BRIEF_MARKER` import. |
| `app/components/chat/ChatBox.tsx` | Additive: renders `<ModelTierPanel /> + <ModelTierPill />` (net-new; locked during creation and under the threshold). |
