...
2. Server assembles the request: active prompt version (cached prefix, §4.3) — which includes the skills index (§4.11) — + project context (file manifest, relevant file contents, recent Vite/terminal errors surfaced by the client) + conversation history + the skill tools (`load_skill`, `read_skill_resource`, §4.11).
   > ⚠️ **The history is NOT windowed, and this is the largest known open cost item (audited 2026-07-13).** This spec previously claimed "windowed conversation history"; it was never built. `proxy.ts` sends the full history every turn, and because all four cache breakpoints sit on the **system** blocks, the history falls *outside* the cached prefix — so it is re-sent at **full uncached input rate on every turn and grows monotonically**. The last-3 slice / `createSummary` / `selectContext` machinery lives in upstream's `/api/chat`, which is now dead code (§4.5.4). See `spec/context-budget.md` §"Levers that are NOT built".
...
**Open levers, in order:** (1) **window the conversation history** — see the warning on §4.2 item 2; it is unwindowed, uncached, and grows every turn, and it is now the biggest known saving left. (2) **Model routing** (Haiku for trivial turns, 3× cheaper). *(Shipped since this section was written: the patch/diff action type — `type="edit"`, `app/lib/runtime/edit-blocks.ts` — and the waste diagnostics above.)*
...
1. ~~Golden starter template~~ — decided: `babylontoolkit/StarterAssets` (ES6 by design; MIT; template repo). ~~Residual: submodule vendoring in snapshots~~ — **resolved: the starter vendors `src/babylon` directly; it has no submodules (§4.4).** Residual: WebContainer install time/memory with the full Babylon dependency set.
