...
- **`GitStatusChip` is the ONE deliberate exception**, because it carries STATE rather than an action: §4.5.4b requires the unsynced state to be loud (amber) and the synced state to be quiet, an asymmetry a uniform style cannot express. Its `neutral`/`busy` tones therefore **match `TOOLBAR_BUTTON` exactly** — a healthy project sits in the row looking like every other control, and only amber/red break out. Restyle one, restyle both.
