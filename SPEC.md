...
> **Status: BUILT.** The user's game code lives in their own GitHub/GitLab repo; the platform holds the project record + chat and no files. §4.13's GitHub Sync is no longer an optional bridge — it IS the storage backbone. **Nine deviations from the design above, each with a reason — read them before changing this area:**
>
