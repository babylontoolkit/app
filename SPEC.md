...
**Still to build:** a second provider. E2B / Cloudflare Sandbox / Daytona / self-hosted Firecracker all now implement one interface rather than a refactor of the app. Costs shift from $0 to per-session compute → a `SANDBOX_RATE` term in the credit formula, and note it accrues **while the user is idle**, which is a billing shape the ledger has never had (§4.6). The binary-bytes contract (§1.3 principle 10) also gains a network hop per read. Ship behind a per-env flag; A/B before cutover.
