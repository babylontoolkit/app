...
**UI surface (required):** LINKED/UNLINKED indicator on project header + dashboard cards (UNLINKED = amber "Not saved — browser only"; LINKED = provider icon + repo@branch, quiet). Unsaved-work nudges — **milestone-based, never timed nags, never blocking generation**: a **long, loud save-reminder toast after EVERY new project's first creation** (per PROJECT, not once per user — every unlinked game is one cleared-cache or device-switch from lost, so each one earns the reminder; `saving.introToastAutoCloseMs`, default 60s, or `false` to require a dismissal — keyed `bt_saving_intro_shown:<projectId>`); dismissible banner every N generations (default 5) while unlinked; `beforeunload` warning with unsynced work; Save always one click in the header. Help page `/help/saving-projects` in plain non-developer language (what GitHub/GitLab are, why the game lives in THEIR account, the VS Code round-trip).
