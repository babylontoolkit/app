...
Key property of this architecture: **compute for running user projects costs us ~nothing** (WebContainers = user's browser). Our marginal costs are LLM tokens (funded by prepaid credits) and S3/CloudFront + Supabase database usage (small). ⚠️ **This claim is provider-conditional since 2026-07-27:** a CodeSandbox build (§8) runs the project on a platform-billed microVM (~$0.074/hr Pico) — unmetered until §8's cost model ships, folded into the margin as a documented placeholder meanwhile (CREDITS.md §"Sandbox compute").
...
| Claude Opus 5 (platform default, 2026-07-27) | `claude-opus-5` | 1,000,000 | 128,000 |
...
|---|---|---|
| `ANTHROPIC_API_KEY` (Console org) | Credits-mode generation (`DEFAULT_MODEL` = `claude-opus-5`) | Credits UI renders; generation returns "platform key not configured"; devs set `PRO_FEATURES_ENABLED=true` + BYOK locally |
