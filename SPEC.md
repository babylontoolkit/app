...
The landing page is not the only starter surface that ships Babylon-branded. On a new project the agent also redesigns the game's **chrome** in `src/babylon/custom/**` to the SAME design — the starter's default splash, preloader, and overlay carry the Babylon logo/spinner and placeholder copy, and §2.3 forbids that branding reaching the user's game exactly as it does on the landing page. This is a **restyle that preserves wiring**, not a free-for-all — each surface is functional:
- **Preloader** — `src/babylon/custom/loading.tsx`. Shown before the app mounts. It **re-exports `babylonLogo`/`spinnerLogo`** that `splash.tsx` imports — a rewrite keeps those exports (or updates `splash.tsx`'s import). `public/babylon.png` + `public/spinner.png` stay (framework-required, §4.4), whatever the new design displays.
- **Splash / loading screen** — `src/babylon/custom/splash.tsx` + `splash.css`. Shown while the 3D scene loads. **Keeps the `GameManager.EventBus` `"OnLoadProgress"` subscription and its status text** — real load progress, not decoration.
- **Initial overlay** — `src/babylon/custom/overlay.tsx` + `overlay.css`. The in-game HUD layer, shipped as a placeholder. Gets a minimal, themed starting point (a title/brand corner, a frame); the full HUD grows later with the gameplay features (§4.4c growth path). **Keeps `pointer-events: none` on the container** so input reaches the canvas — only genuinely interactive elements go `pointer-events: auto`.
- **Bundle-integrity nuance:** unlike `src/pages`/`src/components` (which must stay Babylon-free — see below), `src/babylon/custom/**` renders in the viewer context, so it MAY import `GameManager`/`EventBus` for game data and `useUnifiedNavigation` for navigation (`splash.tsx` already imports `GameManager`). The read-only list below does NOT include `custom/**` — it is a write zone.
...
### 4.16 Built-in Image & Video Generation (KIE media) — PRICING FOUNDATION BUILT 2026-07-18

> **Status: the pricing foundation is BUILT** (the §4.6 Marketplace price list carries the media price table, admin-controlled, with `lookupMediaPrice` refusing anything unpriced); **the generation surface is the next build.** Owner directive (2026-07-18): platform-native media generation like Lovable/seels.ai — no external MCP connector — riding the same `KIE_API_KEY` the LLM provider uses, billed as **additional credits** at the same `CREDIT_MARGIN`.

- **Tools the agent surfaces:** `generate_image`, `generate_video`, `generate_google_video` (wire reference: `temp/kie-image-mcp` — KIE's `/api/v1/jobs/createTask` + poll for image/video, the flat-body `/api/v1/veo/generate` for Veo 3.1, and the base64 upload endpoint for reference images). Defaults: `nano-banana-2` (image), `kling-3.0/video` (video), `veo3_fast` (Google). **Async-enqueue semantics:** a tool call debits + creates the KIE task + returns the destination path immediately; the client polls and writes the bytes when the render lands — a multi-minute render must never hold the LLM tool loop open (§4.2.8).
- **Billing is up-front, refund-on-failure — the inverse of LLM settlement.** Media prices are fixed and known before the task runs (model × options × duration, from the Marketplace price list), so the exact retail credits (`ceil(usd / CREDIT_UNIT_COST × CREDIT_MARGIN)`) are debited BEFORE the KIE call; failures auto-refund through the ledger. An unpriced model/option combo is REFUSED (no most-expensive fallback — that direction is only safe after spend). Each task gets its own `media_generations` record + ledger entry kind so admin usage reporting separates media spend from LLM spend.
- **Bytes land in the PROJECT** (`public/assets/generated/…`, WebContainer FS as `Uint8Array` — the binary-first-class path, never a `boltArtifact`): KIE's result URLs expire (~3d image / ~14d video), so the server proxies the download and the file rides the user's repo like any other asset (§4.5.4b).
- **UI:** a workbench Media panel — prompt + dropdowns (model, aspect ratio, resolution 1K/2K/4K or 720p/1080p/4k, duration, sound), the **credit price shown before Generate** (it is exact), progress, result in the file tree; per-project defaults the agent's tool calls inherit.
