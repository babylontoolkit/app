...
`project-installer.md` defines a BLOCKING platform-detection procedure (Lovable / Replit / Bolt.new / Base44 / V0 / Generic), each mapping to a platform-specific reference doc. Our builder is a **distinct host platform** — a bolt.diy fork with its own runtime, file zones, and creation flow. **Action (owner, agent repo):** author a platform row + reference doc (e.g. `references/web-app-babylon-builder.md`) covering: AppTemplate clone (self-contained — `src/babylon` is vendored, NO submodules), remote-origin removal, ES6 package set, our §4.4b file zones (`src/scripts` write zone; `babylon/classes` + `babylon/system` + router read-only), the §4.4c play contract + bundle-integrity rule, and the required `public/` binary assets. Until it exists, bake **Generic** (`web-app-generic.md`) behavior. **Baked-prompt corollary:** the detection procedure's "fetch the doc" step is satisfied at doc-sync time, not per generation — the agent must NEVER be instructed to fetch platform docs at runtime (§1.3 principle 3).
...
- **`public/babylon.png` + `public/spinner.png` MUST exist in every project (framework requirement — do not "optimize" this away).** They are ALSO ESM-imported by the preloader (`src/babylon/custom/loading.tsx` does `import babylonLogo from '../assets/babylon.png'`), so they exist in two places on purpose: the bundled copy under `src/babylon/assets/` **and** a runtime copy under `public/`. The starter does not ship the `public/` copies, so **project creation copies them from `src/babylon/assets/` into `public/`** and verifies both are present before declaring the project ready — along with verifying that no imported binary is absent. These two files were the observed casualties of the binary bug and are the canonical smoke test that the file layer is honest.
- **⚠️ DIVERGENCE — the built path is a LIVE fetch, not the pinned snapshot this section describes (Stage 5 / §5 hardening item; be honest about it).** "Mount the starter *snapshot*" is the TARGET wording; the code (`app/routes/api.github-template.ts`) actually fetches the AppTemplate zipball **live from GitHub on every New Project** — the **default-branch (`main`) zipball**, because AppTemplate publishes no releases. Consequences of the current default (kept deliberately for now): a push to `main` reaches the *next* new project immediately with no sync/rebuild step; but there is **no pin, no cache, no rollback, and `toolkit_version` is metadata that is not enforced** against what `main` actually contains, so a broken push to `main` breaks every new project at once, and creation carries a runtime GitHub-availability dependency.
  - **Safety net — BUILT (2026-07-14):** a **last-known-good template snapshot** (`app/lib/.server/templates/last-known-good.ts`, tested). Every successful, structurally-valid fetch is persisted to object storage (the same S3/local-FS layer as §4.5.5); a later fetch that fails outright **or** returns an unmountable result (missing `package.json`, or missing vendored `src/babylon/**` — the exact §4.4 breakages) serves the most recent good snapshot instead of failing project creation. A `?fallback=1` query param lets the client force the snapshot when it detects a broken mount at runtime. The response carries `X-Template-Source: live | last-known-good` for diagnostics. This is the small, high-value net beneath live-`main` — **NOT** the pin-and-cache design below.
  - **TARGET — pin-and-cache (deferred §5 hardening; spec, not yet built):** fetch once → write a byte-faithful **S3 template snapshot** → pin the registry's `toolkit_version` to a specific **commit SHA / release** → new projects mount the **PINNED snapshot, not live `main`** → **promotion is a deliberate admin action** (re-point the pin to a newer snapshot), with **one-click rollback** to any prior snapshot when a push is bad. This removes the creation-time GitHub dependency and gives real version isolation between what is on `main` and what new projects get.
  - **⚠️ RELEASE-LOCK FOOTGUN:** `api.github-template.ts` prefers `releases/latest` and only falls back to the `main` zipball on a `404`. **Publishing ANY GitHub Release on `babylontoolkit/AppTemplate` flips creation to release-locked** — new projects then mount `releases/latest` and `main` pushes STOP flowing until a *new* release is cut. Today (no releases) creation tracks live `main`; do not publish a Release on AppTemplate unless you intend that switch.
