...
> 🔴 ⚠️ **RETRACTED 2026-08-12. This reasoning was wrong and it cost real money — do not reuse the pattern.** Anthropic made `$2/$10` the STANDARD price and cancelled the 1 September increase, so Comet's feed was reporting the current rate all along and `MODEL_RATES` sat 1.5× high on the platform's default model for weeks. Because credits are cost-proportional, that over-charge was **passed through to users**, and it inflated `savings.ts`' advertised discount from a true 20% to ~47%.
>
> The parenthetical was the closest thing to a safety net — *"after 2026-08-31 the row simply becomes the plain list price"* — and it was wrong in the one way that mattered: it assumed the deviation would end by the price RISING to meet our table, so nobody scheduled a check for the price staying put. **A dated carve-out needs an owner and a review date, not a prediction about which way the vendor will move.**
>
> The deeper error was epistemic: an independently CAPTURED source disagreed with a hand-maintained table, and the disagreement was resolved in the table's favour because a plausible story was available — one that could not be falsified from inside this repo without reading the vendor's page. **When a capture contradicts a hand-maintained value, the burden of proof belongs on the hand-maintained value.**
...
- [x] **T5** — Capture Comet prices with the ratio applied; ship only live-probed ids
  - ⚠️ **EXECUTION FINDING (2026-08-10) — the spec was wrong and FR4 caught it.** `claude-haiku-4-5`,
    ...
    contradicting it. Nothing was changed.** 🔴 **RE-OPENED AND FIXED 2026-08-12: that closure was wrong —
    $2/$10 became the standard price, the September increase was cancelled, and "nothing was changed" left a
    1.5× over-charge on the platform default model in place. See the retraction at OQ1 above.** All findings written back into
