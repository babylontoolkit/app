...
- [x] **T8** — Carry the prompt into the textbox
...
- [x] **T9** — New Project mode: the visible surface
...
- [x] **T10** — The hidden brief, appended on send
...
- [x] **T12** — Split the celebration: project ready vs. game ready
...
- [x] **T13** — Keep the ten protections, prove each one
...
- [x] **T14** — Rewrite `NEW_PROJECT.md` for the new contract
...
- [x] **T16** — The handoff card (added 2026-07-29 after driving the shipped flow)
  - Files: `app/components/chat/CreationHandoffCard.tsx` (replaces `NewProjectBanner.tsx`), `app/lib/chat/creation-handoff.ts`, `app/lib/chat/new-project-draft.ts`, `app/lib/stores/new-project-mode.ts`, `app/components/chat/Chat.client.tsx`, `app/components/chat/BaseChat.tsx`, `SPEC.md`, `NEW_PROJECT.md`, `CLAUDE.md`
  - Details: T8/T9 shipped the handoff IMPLICITLY — the prompt reappeared in the chat box and a banner described the screen. Driving it found that reading as leftover state (owner: *"it kind of feels disconnected to the initial project creation process"*), plus two live defects the 3,616-test suite could not see: a card click DISCARDED typed words, and creation uploaded no conversation at all. Replace the banner with a card offering **Build my game** / **Edit my brief** / **X**, carry the words on the mode (`userPrompt`, persisted; the `cachedPrompt` cookie retired from this path), and checkpoint the fresh project.
  - Acceptance: creation leaves the box empty and writes no prompt cookie; the card shows the brief and its Build goes through the ordinary `sendMessage` (one `/api/agent` turn carrying `CREATION_BRIEF_MARKER`); X hides the card without leaving the mode, persisted across a reload; a card path with no words offers no Build; gates green; the flow re-driven in a real browser.
