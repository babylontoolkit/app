...
function CurrentDateTime() {
  ...
  /*
   * Lives in the FOOTER (2026-08-02), centred, in the slot the wordmark vacated. It used to sit
...
/**
 * The sidebar's user bar (2026-08-02): the ROW the date/time used to occupy, directly under the
 * gradient strip — same container, same border — now dedicated to who you are. Name left, avatar
 * right; the clock moved to the footer's centre slot. It moves WITH the sidebar, so unlike the
 * header-pinned identity it replaced there is no animation seam for it to flash on.
 *
 * `identity.name` is EMPTY while `/api/me` is in flight, deliberately (`~/lib/identity`): asserting a
 * name we have not been told and swapping it a moment later is the defect that module exists to
 * remove. The avatar well renders regardless, so the row does not reflow when the name arrives.
 */
function SidebarIdentity() {
  const identity = useDisplayIdentity();

  return (
    <div className="flex items-center justify-between gap-3 px-4 py-2 border-b border-gray-100 dark:border-gray-800/50">
      <div className="flex flex-col min-w-0">
        <span className="font-medium text-sm truncate text-gray-900 dark:text-gray-100">{identity.name}</span>
        {identity.secondary && (
          <span className="text-[11px] truncate text-gray-500 dark:text-gray-500">{identity.secondary}</span>
        )}
      </div>

      <div className="flex items-center justify-center w-8 h-8 overflow-hidden bg-gray-100 dark:bg-gray-800 rounded-full shrink-0">
        {identity.avatar ? (
          <img
            src={identity.avatar}
          ...
          <div className="i-ph:user-fill text-base text-gray-500 dark:text-gray-400" />
        )}
      </div>
    </div>
  );
    ...
    <>
      ...
      <div className="contents" data-theme="dark">
        ...
        >
          ...
          >
            {/*
             * Deliberately EMPTY. When the sidebar is docked this strip sits beside the header at the
             * same height with the same gradient, so the chrome band reads as one continuous bar
             * across the top of the screen — that continuity is its whole job. The user bar is the
             * ROW BELOW (`SidebarIdentity`, where the date/time used to be), not this strip.
             */}
            {/*
             * Hidden (hide-don't-delete, the Deploy "Coming Soon" precedent — §4.1a). Documentation
             * is not orphaned: it is one click away in the avatar dropdown's "Help & Documentation"
             * item and in the ⋯ main menu's Help & diagnostics group.
             */}
          ...
          <div className="flex-1 flex flex-col h-full w-full overflow-hidden">
            ...
            {/*
             * The footer's centre slot holds the CLOCK (2026-08-02; it held the wordmark for a few
             * hours, which went back to the page header where it started). The date/time used to be a
             * full-width row directly under the top strip — a clock sitting above the chat list, which
             * is the panel's actual job.
             *
             * **A 3-column grid, not `justify-between`.** With three children `justify-between`
             * centres the middle one only while the two side controls happen to be the same width.
             * They are today (both `IconButton size="xl"`), which is exactly what makes it a trap:
             * the left slot is a flex row built to hold more than one button, and the day a second
             * one lands there the centre slides off with nothing to explain why. `1fr auto 1fr`
             * centres it by construction.
             */}
