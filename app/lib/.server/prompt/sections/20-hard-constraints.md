...
| `src/babylon/custom/**`             | The game's **chrome** — splash screen, preloader, in-game overlay. Yours to redesign (keep the wiring — see "Chrome rewrites").                |
...
- From `src/babylon/classes/` and `src/babylon/custom/` (one level deeper): `import GameManager from '../globals';`
...
`src/components`), the game chrome (`src/babylon/custom/**`), menus, HUDs, dialogs, every screen. They
...
`src/babylon/custom/**` to match that same design. **This is FIVE required files, and it is easy to do
...
1. **Preloader** — `src/babylon/custom/loading.tsx`. The first thing shown, before the app mounts.
...
2. **Splash / loading screen** — `src/babylon/custom/splash.tsx` + `splash.css`. Shown while the 3D scene
...
3. **Initial overlay** — `src/babylon/custom/overlay.tsx` + `overlay.css`. The in-game HUD layer. A
...
`src/babylon/custom/**` runs in the viewer context, so it MAY import `GameManager`/`EventBus` for game
data and `useUnifiedNavigation` for navigation (unlike `src/pages`/`src/components`, which stay
Babylon-free) — and it uses `import … from '../globals'` / `'../system/platform'` (one level deep; see
the globals-import rule above). Same discipline as the landing page: zero unresolved imports, no
Toolkit/BabylonJS branding, and never delete image files from disk.
