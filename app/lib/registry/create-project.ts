...
**Your task now**
1. Rewrite \`src/pages/Home.tsx\` and \`src/pages/Home.css\` COMPLETELY, as a landing page designed from scratch for *${title}*. Nothing from the starter page survives — no hero montage, no demo buttons, no Vite/React/Babylon links, no footer, no attribution of any kind. Reach gameplay through the play contract above. **FULL-PAGE-WIDTH — this is the Layout law, not a preference:** the page fills the entire viewport edge-to-edge like a game console dashboard; root and section containers are \`width: 100%\`/\`100vw\` with full-bleed backgrounds, heroes, and bars. Do NOT wrap it in a centered fixed-width column (\`max-width: …px; margin: 0 auto\`) — a game landing page is NOT an article, so do not reach for the editorial/reading-column escape hatch here. Inner text may still be constrained for readability *inside* a full-bleed section; the section itself stays edge-to-edge. Responsive from ~320px to ~2560px, no horizontal scrollbar at any width.
2. REDESIGN — do not reskin — the game chrome in \`src/babylon/custom/**\`, all THREE surfaces, not just the overlay: the preloader (\`loading.tsx\`), the splash / loading screen (\`splash.tsx\` + \`splash.css\`), and the initial overlay (\`overlay.tsx\` + \`overlay.css\`). **The splash must NEVER be derived from the default Babylon splash (centered logo + spinner) — recoloring the default IS the failure.** Be creative and think out of the box: design the loading experience as a scene in *${title}*'s world, with a progress metaphor native to this game (a racer's start-lights counting down, a gauge filling, a level assembling — whatever fits THIS game), the same typography/palette/motion as your landing page, and atmosphere worth watching while it loads. There is no limit to what you can do here. Keep ONLY each surface's wiring (the \`babylonLogo\`/\`spinnerLogo\` re-exports, the \`OnLoadProgress\` subscription, \`pointer-events: none\` on the overlay container) — see "Chrome rewrites" for the details; replace ALL of the visuals. The splash and preloader ship the Babylon logo + spinner, so skipping them leaves BabylonJS branding in the user's game.
3. ${
    prompt
