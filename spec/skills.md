...
With tools off, the same edit takes **29–39s** and produces a correct patch. The decision is the pure `toolPolicyForTurn` (`agent/tool-policy.ts`, 2026-07-18 — previously the inline `allowTools = !isCreationTurn && preloaded.length === 0 && !slash`): ordinary turns open the full loop only when nothing is preloaded and no `/slash` ran (MCP tools force it open, §4.14); a CREATION turn gets a media-only loop when §4.16 tools exist (skill tools never — the pathology above stays dead) and the historic one-shot otherwise. Tools exist only for the turn the keyword router could not anticipate, the only turn where they can help.
